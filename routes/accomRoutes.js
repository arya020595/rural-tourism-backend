...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!finalName || !finalUserId) {
      return res
        .status(400)
        .json({ error: "name (or homest_name) and user_id are required." });
    }
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Parse available dates if provided
    const rawDates = available_dates || availableDates;
    if (rawDates !== undefined) {
      let availableDatesArray = [];
      if (typeof rawDates === "string") {
        try {
          availableDatesArray = JSON.parse(rawDates);
        } catch {
          availableDatesArray = [];
        }
      } else if (Array.isArray(rawDates)) {
        availableDatesArray = rawDates;
      }
      accommodation.available_dates = availableDatesArray;
    }

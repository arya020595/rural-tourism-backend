...
function buildHtml(data, pdfUrl) {
  const {
    id, touristFullName, companyName, accommodationName,
    checkInDate, checkOutDate, totalNight, totalRooms,
    status, operatorName, operatorEmail, totalPrice,
    createdAt, companyLogoBase64,
  } = data;
    ...
    <div class="receipt-body">
      <div class="details-grid">
        ...
        <div class="receipt-row">
          ...
          <div class="row-value">${escapeHtml(formatDate(checkInDate))}</div>
        </div>
        <div class="receipt-row">
          <div class="row-label">DAFTAR KELUAR/<em>CHECK-OUT</em></div>
...
async function generateAccommodationReceiptPdf(data, pdfUrl) {
  ...
  try {
    ...
    console.error("Accommodation receipt PDF error:", { message: error?.message, stack: error?.stack });
    throw error;
  } finally {
    await page.close();
  }
